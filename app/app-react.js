(function(){
  ...
  class App extends React.Component{
    ...
    renderSessionCommand(){
      var asrReady=this.state.asr&&this.state.asr.enabled&&!/FAILED|ERROR|AUTH_REQUIRED/.test(String(this.state.asr.lastState||''));
    ...
    renderTranscript(){
      var latest=this.state.transcripts[0];
      var recent=this.state.transcripts.slice(0,4);
      var asrReady=this.state.asr&&this.state.asr.enabled;
      return h(Surface,{className:'transcript-surface focus-transcript'},
        h(SectionHead,{eyebrow:'LIVE TRANSCRIPT',title:'متن زنده',subtitle:'فقط رونویسی صوت؛ پاسخ‌ها در این بخش تکرار نمی‌شوند.',action:h('div',{className:'transcript-head-actions'},h(Chip,{tone:latest&&latest.text_raw?'success':asrReady?'warning':'neutral',dot:true,active:Boolean(latest&&latest.text_raw)},latest&&latest.text_raw?'FINAL':asrReady?'LISTENING':'ASR OFF'),h(Button,{variant:'soft',tone:'neutral',disabled:!this.state.transcripts.length,onClick:this.openTranscriptArchive.bind(this)},'متن کامل'))}),
        recent.length?h('div',{className:'live-transcript-feed'},recent.map(function(item,index){return h('article',{key:item.segment_id,className:'live-transcript-row'+(index===0?' newest':'')},h('div',{className:'live-transcript-row-meta'},h(Chip,{tone:sourceRoleFromChannel(item.channel_id)==='system'?'purple':'blue'},roleLabel(sourceRoleFromChannel(item.channel_id))),h('span',null,formatTime(item.created_at||item.updated_at||new Date().toISOString())),index===0?h('span',{className:'live-now'},'جدیدترین'):null),h('div',{className:'live-transcript-copy'},text(item.text_raw,item.asr_error?'خطا: '+text(item.asr_error):'در حال پردازش…')));},this)):h('div',{className:'latest-transcript empty-live'},asrReady?'در حال شنیدن… پس از تثبیت گفتار، متن اینجا ظاهر می‌شود.':'ASR را از نوار کنترل یا تنظیمات فعال کن.'),
    ...
    renderSettings(){
      var asr=this.state.asr||{},ready=asr.enabled&&!/FAILED|ERROR|AUTH_REQUIRED/.test(String(asr.lastState||''));
