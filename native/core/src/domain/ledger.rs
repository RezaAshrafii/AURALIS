...
impl CaptureState {
    ...
    pub fn from_storage_str(value: &str) -> Result<Self, &'static str> {
        match value {
            "IDLE" => Ok(Self::Idle),
    ...
    pub fn can_transition_to(self, next: Self) -> bool {
        self == next
            || matches!(
                (self, next),
                (Self::Idle, Self::Starting | Self::Recovering)
                    | (
                        Self::Starting,
                        Self::Capturing | Self::Stopped | Self::Recovering | Self::Failed
                    )
                    | (
                        Self::Capturing,
                    ...
                    | (
                        Self::Stopping,
                        Self::Stopped | Self::Recovering | Self::Failed
                    )
...
impl RecoveryState {
    ...
    pub fn from_storage_str(value: &str) -> Result<Self, &'static str> {
        match value {
            "CLEAN" => Ok(Self::Clean),
            "SCAN_REQUIRED" => Ok(Self::ScanRequired),
        ...
        self == next
            || matches!(
                ...
                (Self::Clean, Self::ScanRequired)
                    ...
                    | (
                        Self::Recoverable,
                        Self::ScanRequired | Self::Recovered | Self::ManualIntervention
                    )
...
impl AudioChunkState {
    ...
    pub fn from_storage_str(value: &str) -> Result<Self, &'static str> {
        match value {
            ...
            _ => Err("unknown audio chunk state"),
        }
    }
}
...
impl GapReason {
    ...
    pub fn from_storage_str(value: &str) -> Result<Self, &'static str> {
        match value {
            "WASAPI_DATA_DISCONTINUITY" => Ok(Self::WasapiDataDiscontinuity),
            "TIMESTAMP_ERROR" => Ok(Self::TimestampError),
            "DEVICE_POSITION_GAP" => Ok(Self::DevicePositionGap),
            "QUEUE_OVERFLOW" => Ok(Self::QueueOverflow),
            "DEVICE_INVALIDATED" => Ok(Self::DeviceInvalidated),
            "SPOOL_WRITE_FAILURE" => Ok(Self::SpoolWriteFailure),
            "RECONNECT" => Ok(Self::Reconnect),
            "RECOVERY_TRUNCATION" => Ok(Self::RecoveryTruncation),
            "UNKNOWN" => Ok(Self::Unknown),
            _ => Err("unknown gap reason"),
        }
    }
}
...
impl AudioChannel {
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        if self.bits_per_sample == 0
            ...
            || self.block_align == 0
        {
            return Err("channel sample layout is invalid");
        }
        Ok(())
    }
}
...
impl AudioChunk {
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        if self.bits_per_sample == 0
            ...
            || self.block_align == 0
        {
            return Err("chunk sample layout is invalid");
        }
        ...
        } else if !self.sha256_hex.is_empty() {
            return Err("non-finalized chunk must not claim a SHA-256 digest");
        }
