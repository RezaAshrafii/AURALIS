...
function parsePort(value) {
  ...
  if (!Number.isSafeInteger(port) || port < MIN_UNPRIVILEGED_PORT || port > MAX_PORT) {
    throw new TypeError(`AURALIS_PORT must be an integer between ${MIN_UNPRIVILEGED_PORT} and ${MAX_PORT}`);
  }
  return port;
}
...
export async function loadRuntimeConfig(root, env = process.env) {
  ...
  return Object.freeze({
    ...
    sourceApp,
    app: env.AURALIS_USE_WEB_BUILD === '1' || env.AURALIS_USE_VITE_BUILD === '1'
      ? join(root, 'dist', 'web')
      : sourceApp,
    data,
