...
export function createLocalRequestGuard({ host, port, token }) {
  ...
  const safeHost = request => allowedHosts.has(String(request.headers.get('host') || '').toLowerCase());
  ...
  return Object.freeze({
    ...
    stateChangeAllowed: request => safeHost(request) && sameOrigin(request) && authenticated(request)
  });
}
...
export async function readJsonBody(request, maxBytes = 1_048_576) {
  ...
  if (contentType && !contentType.startsWith('application/json')) {
    throw new HttpInputError('UNSUPPORTED_MEDIA_TYPE', 415, 'Content-Type must be application/json');
  }
...
export function resolveStaticPath(root, pathname) {
  ...
  if (decoded.includes('\0')) throw new HttpInputError('INVALID_PATH', 400, 'URL path contains a null byte');
