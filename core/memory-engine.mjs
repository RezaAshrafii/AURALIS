...
import { MemoryCommandPort, MemoryQueryPort } from "./memory-ports.mjs";
...
const INJECTION_PATTERN =
  /ignore\s+(all|previous)|system\s+prompt|دستور\s+سیستم|نادیده\s+بگیر|developer\s+message|جیل\s*بریک/i;
  ...
  extract({ turns, people, projects }) {
    ...
    for (const turn of turns) {
      ...
      if (/جواب|پاسخ/.test(quote) && /کوتاه|مختصر/.test(quote)) {
        candidates.push({
          scopeType: "USER",
          scopeRef: "default-profile",
          memoryType: "PREFERENCE",
          canonicalKey: "response.length",
          content: "پاسخ‌ها کوتاه و مستقیم باشند.",
          confidence: 0.94,
          validFromText: null,
          validUntilText: null,
          sensitivity: "NORMAL",
      ...
      if (/فارسی/.test(quote) && /رسمی/.test(quote)) {
        candidates.push({
          ...
          scopeRef: "default-profile",
          memoryType: "PREFERENCE",
          canonicalKey: "response.language-style",
          content: "پاسخ‌ها به فارسی رسمی نوشته شوند.",
          confidence: 0.94,
          validFromText: null,
          validUntilText: null,
          sensitivity: "NORMAL",
      ...
      for (const project of projects) {
        ...
        if (/وضعیت|متوقف|فعال|تمام|تکمیل|مهلت|بودجه|اولویت/.test(quote)) {
          candidates.push({
            ...
            validFromText: null,
            validUntilText: null,
            sensitivity: SENSITIVE_PATTERN.test(quote) ? "SENSITIVE" : "NORMAL",
            evidence,
          });
        }
      }
...
export class MemoryEngine extends MemoryQueryPort {
  ...
  getSettings(workspaceId) {
    const row = this.db
      .query("SELECT * FROM memory_settings WHERE workspace_id = ?")
      .get(workspaceId);
  ...
  _assertWorkspace(workspaceId) {
    if (!this.db.query("SELECT id FROM workspaces WHERE id = ?").get(workspaceId))
  ...
  _assertScope(workspaceId, scopeType, scopeId) {
    ...
    let row;
    if (scopeType === "USER")
      row = this.db.query("SELECT id FROM local_profiles WHERE id = ?").get(scopeId);
    if (scopeType === "WORKSPACE") row = scopeId === workspaceId ? { id: scopeId } : null;
    if (scopeType === "PERSON")
      row = this.db
        .query("SELECT id FROM people WHERE id = ? AND workspace_id = ?")
        .get(scopeId, workspaceId);
    if (scopeType === "PROJECT")
      row = this.db
        .query("SELECT id FROM projects WHERE id = ? AND workspace_id = ?")
        .get(scopeId, workspaceId);
  ...
  listMemories(
    workspaceId,
    { scope = null, type = null, status = null, query = null, cursor = 0, limit = 50 } = {}
  ) {
    ...
    if (scope) {
      ...
      params.push(String(scope).toUpperCase());
    }
    if (type) {
    ...
    if (query) {
      sql += " AND (r.content_text LIKE ? OR m.canonical_key LIKE ?)";
      const like = `%${normalizeContent(query)}%`;
  ...
  getMemory(workspaceId, id, { includeDeleted = false } = {}) {
    const row = this.db
      .query(`SELECT m.*,r.content_text,r.revision memory_revision
      FROM memory_items m LEFT JOIN memory_revisions r ON r.id=m.current_revision_id
      WHERE m.id=? AND m.workspace_id=?`)
      .get(id, workspaceId);
    if (!row || (!includeDeleted && row.status === "DELETED")) return null;
    const item = mapItem(row);
    item.revisions = this.db
      .query("SELECT * FROM memory_revisions WHERE memory_id=? ORDER BY revision DESC")
      .all(id)
      .map((revision) => ({
        id: revision.id,
        revision: revision.revision,
        content: revision.content_text,
        contentJson: JSON.parse(revision.content_json || "{}"),
        reason: revision.reason,
        createdBy: revision.created_by,
        provider: revision.provider,
        providerModel: revision.provider_model,
        promptVersion: revision.prompt_version,
        inputFingerprint: revision.input_fingerprint,
        createdAt: revision.created_at,
        evidence: this.db
          .query("SELECT * FROM memory_evidence WHERE memory_revision_id=? ORDER BY created_at")
          .all(revision.id)
          .map((ev) => ({
            id: ev.id,
            ...
            documentChunkId: ev.document_chunk_id,
    ...
    item.contradictions = this.db
      .query(`SELECT * FROM memory_contradictions
      WHERE workspace_id=? AND (left_memory_id=? OR right_memory_id=?) ORDER BY created_at DESC`)
      .all(workspaceId, id, id)
      .map(this._mapContradiction);
    item.purgeJob =
      this.db.query("SELECT * FROM memory_purge_jobs WHERE memory_id=?").get(id) || null;
    return item;
  }

  listUsage(workspaceId, memoryId) {
    const item = this.db
      .query("SELECT id FROM memory_items WHERE id=? AND workspace_id=?")
      .get(memoryId, workspaceId);
    if (!item) throw new Error("MEMORY_NOT_FOUND");
    return this.db
      .query("SELECT * FROM memory_use_audits WHERE memory_id=? ORDER BY created_at DESC LIMIT 100")
      .all(memoryId)
      .map((row) => ({
        id: row.id,
        ...
        rank: row.rank,
        score: row.score,
        includedChars: row.included_chars,
        createdAt: row.created_at,
      }));
  }
  ...
  listBackfillJobs(workspaceId) {
    ...
    return this.db
      .query(
        "SELECT * FROM memory_backfill_jobs WHERE workspace_id=? ORDER BY created_at DESC LIMIT 20"
      )
      .all(workspaceId)
      .map(this._mapBackfillJob);
  }
  getBackfillJob(workspaceId, id) {
    const row = this.db
      .query("SELECT * FROM memory_backfill_jobs WHERE id=? AND workspace_id=?")
      .get(id, workspaceId);
    return row ? this._mapBackfillJob(row) : null;
  }
  startBackfill(workspaceId, { batchSize = 5 } = {}) {
    const settings = this.getSettings(workspaceId);
    if (!settings.enabled || !settings.candidateExtractionEnabled)
      throw new Error("MEMORY_DISABLED");
    const active = this.db
      .query(
        "SELECT * FROM memory_backfill_jobs WHERE workspace_id=? AND state IN ('QUEUED','RUNNING','PAUSED') ORDER BY created_at DESC LIMIT 1"
      )
      .get(workspaceId);
    if (active) return this._mapBackfillJob(active);
    const now = nowIso(),
      id = generateId("mbackfill"),
      safeBatch = Math.max(1, Math.min(25, Number(batchSize) || 5));
    const total = this.db
      .query("SELECT count(*) total FROM conversations WHERE workspace_id=? AND state='READY'")
      .get(workspaceId).total;
    this.db
      .query(`INSERT INTO memory_backfill_jobs
      (id,workspace_id,state,total_count,processed_count,candidate_count,batch_size,created_at,updated_at)
      VALUES (?,?,'QUEUED',?,0,0,?,?,?)`)
      .run(id, workspaceId, total, safeBatch, now, now);
    return this.getBackfillJob(workspaceId, id);
  }
  controlBackfill(workspaceId, id, command) {
    const job = this.getBackfillJob(workspaceId, id);
    if (!job) throw new Error("BACKFILL_JOB_NOT_FOUND");
    const cmd = String(command).toUpperCase(),
      now = nowIso();
    if (cmd === "PAUSE" && ["QUEUED", "RUNNING"].includes(job.state))
      this.db
        .query("UPDATE memory_backfill_jobs SET state='PAUSED',updated_at=? WHERE id=?")
        .run(now, id);
    else if (cmd === "RESUME" && job.state === "PAUSED")
      this.db
        .query("UPDATE memory_backfill_jobs SET state='QUEUED',updated_at=? WHERE id=?")
        .run(now, id);
    else if (cmd === "CANCEL" && !["COMPLETED", "CANCELLED"].includes(job.state))
      this.db
        .query(
          "UPDATE memory_backfill_jobs SET state='CANCELLED',completed_at=?,updated_at=? WHERE id=?"
        )
        .run(now, now, id);
    return this.getBackfillJob(workspaceId, id);
  }
  processBackfillBatch(workspaceId, id) {
    let job = this.getBackfillJob(workspaceId, id);
    if (!job) throw new Error("BACKFILL_JOB_NOT_FOUND");
    ...
    if (!settings.enabled || !settings.candidateExtractionEnabled) {
      this.db
        .query(
          "UPDATE memory_backfill_jobs SET state='PAUSED',error_code='MEMORY_DISABLED',updated_at=? WHERE id=?"
        )
        .run(nowIso(), id);
      return this.getBackfillJob(workspaceId, id);
    }
    this.db
      .query(
        "UPDATE memory_backfill_jobs SET state='RUNNING',error_code=NULL,updated_at=? WHERE id=?"
      )
      .run(nowIso(), id);
    const rows = this.db
      .query(`SELECT id,COALESCE(ended_at,started_at) cursor_time FROM conversations
      WHERE workspace_id=? AND state='READY' AND (? IS NULL OR COALESCE(ended_at,started_at)>? OR (COALESCE(ended_at,started_at)=? AND id>?))
      ORDER BY COALESCE(ended_at,started_at),id LIMIT ?`)
      .all(
        workspaceId,
        job.cursorEndedAt,
        job.cursorEndedAt,
        job.cursorEndedAt,
        job.cursorConversationId || "",
        job.batchSize
      );
    let candidates = 0,
      lastError = null;
    for (const row of rows) {
      const current = this.getBackfillJob(workspaceId, id);
      if (current.state !== "RUNNING") return current;
      try {
        const result = this.extractMemoryCandidates(workspaceId, row.id, { manual: false });
        candidates += (result.candidates || []).length;
      } catch (error) {
        lastError = String(error.message || error).slice(0, 120);
      }
      this.db
        .query(`UPDATE memory_backfill_jobs SET processed_count=processed_count+1,candidate_count=candidate_count+?,
        cursor_ended_at=?,cursor_conversation_id=?,error_code=?,updated_at=? WHERE id=?`)
        .run(candidates, row.cursor_time, row.id, lastError, nowIso(), id);
      candidates = 0;
    }
    ...
    const now = nowIso();
    this.db
      .query(`UPDATE memory_backfill_jobs SET state=?,completed_at=?,updated_at=? WHERE id=?`)
      .run(done ? "COMPLETED" : "QUEUED", done ? now : null, now, id);
    return this.getBackfillJob(workspaceId, id);
  }
  confirmMemory(workspaceId, id, options = {}) {
    return this.commands.confirmMemory(workspaceId, id, options);
  }
  rejectMemory(workspaceId, id, options = {}) {
    return this.commands.rejectMemory(workspaceId, id, options);
  }
  editMemory(workspaceId, id, updates, options = {}) {
    return this.commands.editMemory(workspaceId, id, updates, options);
  }
  archiveMemory(workspaceId, id, options = {}) {
    return this.commands.archiveMemory(workspaceId, id, options);
  }
  deleteMemory(workspaceId, id, options = {}) {
    return this.commands.deleteMemory(workspaceId, id, options);
  }
  ...
  queryRelevantMemories(
    workspaceId,
    query,
    { scopeIds = [], conversationId = null, turnId = null, purpose = "SEARCH", limit = null } = {}
  ) {
    const settings = this.getSettings(workspaceId);
    if (!settings.enabled)
      return { enabled: false, requiresMemory: false, memories: [], budget: settings };
    const normalizedQuery = normalizeContent(query);
    const terms = [...new Set(tokenize(normalizedQuery))].slice(0, 12);
    const memorySignal = /یاد|قبلاً|ترجیح|سبک|پاسخ|جواب|فارسی|پروژه|محدودیت|روال|معمولاً/.test(
      normalizedQuery
    );
    if (!terms.length && !memorySignal)
      return { enabled: true, requiresMemory: false, memories: [], budget: settings };
    ...
    if (terms.length) {
      ...
      try {
        rows = this.db
          .query(`SELECT m.*,r.content_text,r.revision memory_revision,bm25(memory_index) lexical_rank
          FROM memory_index
          ...
          ORDER BY lexical_rank ASC,m.updated_at DESC LIMIT 200`)
          .all(match, workspaceId, now, now, settings.sensitiveMemoryEnabled ? 1 : 0);
      } catch {
        rows = [];
      }
    }
    if (!rows.length && memorySignal) {
      rows = this.db
        .query(`SELECT m.*,r.content_text,r.revision memory_revision,0 lexical_rank
        ...
        .all(workspaceId, now, now, settings.sensitiveMemoryEnabled ? 1 : 0);
    }

    const allowedScopes = new Set([workspaceId, "default-profile", ...scopeIds]);
    const queryTerms = new Set(terms);
    const ranked = rows
      .filter((row) => allowedScopes.has(row.scope_id))
      .map((row) => {
        ...
        const score = Number(
          (lexical * 0.55 + Number(row.confidence) * 0.3 + recency * 0.15).toFixed(6)
        );
        return { ...mapItem(row), score };
      })
      .sort(
        (a, b) =>
          b.score - a.score || b.updatedAt.localeCompare(a.updatedAt) || a.id.localeCompare(b.id)
      );
    ...
    if (["ANSWER_CONTEXT", "SEARCH", "SUGGESTION", "EXPORT"].includes(purpose)) {
      for (const item of selected) {
        this.db
          .query(`INSERT INTO memory_use_audits
    ...
    return {
      enabled: true,
      requiresMemory: selected.length > 0,
      memories: selected,
      includedChars: chars,
      budget: settings,
    };
  ...
  assembleMemoryContext(workspaceId, query, options = {}) {
    const result = this.queryRelevantMemories(workspaceId, query, {
      ...options,
    ...
    const lines = result.memories.map((item) => {
      const evidence = this.db
        .query(
          "SELECT turn_id FROM memory_evidence WHERE memory_revision_id=? AND turn_id IS NOT NULL ORDER BY created_at LIMIT 1"
        )
        .get(item.currentRevisionId);
  ...
  exportMemories(workspaceId, format = "BOTH") {
    return this.commands.exportMemories(workspaceId, format);
  }
  getExport(workspaceId, jobId) {
    const row = this.db
      .query("SELECT * FROM memory_exports WHERE id=? AND workspace_id=?")
    ...
    return {
      ...
      state: row.state,
      format: row.format,
      json: row.payload_json ? JSON.parse(row.payload_json) : null,
      markdown: row.payload_markdown,
  ...
  rebuildMemoryIndex(workspaceId = null) {
    if (workspaceId)
      this.db.query("DELETE FROM memory_index WHERE workspace_id=?").run(workspaceId);
    else this.db.exec("DELETE FROM memory_index;");
    const sql = `SELECT m.*,r.content_text FROM memory_items m JOIN memory_revisions r ON r.id=m.current_revision_id WHERE m.status='CONFIRMED'`;
    const rows = workspaceId
      ? this.db.query(`${sql} AND m.workspace_id=?`).all(workspaceId)
  ...
  _mapBackfillJob(row) {
    return {
      ...
      cursorEndedAt: row.cursor_ended_at,
...
class MemoryCommands extends MemoryCommandPort {
  ...
  _audit({
    ...
    const existing = this.db
      .query("SELECT * FROM memory_command_audits WHERE idempotency_key=?")
      .get(key);
    ...
    this.db
      .query(`INSERT INTO memory_command_audits
  ...
  configureMemory(workspaceId, updates, expectedRevision = null) {
    ...
    if (updates.enabled === true && updates.consent !== true && !current.enabled)
      throw new Error("MEMORY_CONSENT_REQUIRED");
    const enabled = updates.enabled === undefined ? current.enabled : Boolean(updates.enabled);
    const extraction =
      enabled &&
      (updates.candidateExtractionEnabled === undefined
        ? current.candidateExtractionEnabled
        : Boolean(updates.candidateExtractionEnabled));
    const autoConfirm = Boolean(
      updates.autoConfirmUserPreferences === undefined
        ? current.autoConfirmUserPreferences
        : updates.autoConfirmUserPreferences
    );
    const sensitive = Boolean(
      updates.sensitiveMemoryEnabled === undefined
        ? current.sensitiveMemoryEnabled
        : updates.sensitiveMemoryEnabled
    );
    const retentionDays =
      updates.retentionDays === undefined
        ...
        : updates.retentionDays === null
          ? null
          : Math.max(1, Math.min(3650, Number(updates.retentionDays)));
    const budgetItems = Math.max(
      1,
      Math.min(30, Number(updates.contextBudgetItems ?? current.contextBudgetItems))
    );
    const budgetChars = Math.max(
      200,
      Math.min(12000, Number(updates.contextBudgetChars ?? current.contextBudgetChars))
    );
    const now = nowIso();
    ...
    this.db
      .query(`UPDATE memory_settings SET enabled=?,candidate_extraction_enabled=?,auto_confirm_user_preferences=?,
  ...
  extractMemoryCandidates(workspaceId, conversationId, options = {}) {
    ...
    const conversation = this.db
      .query("SELECT * FROM conversations WHERE id=? AND workspace_id=?")
      .get(conversationId, workspaceId);
    if (!conversation) throw new Error("CONVERSATION_NOT_FOUND");
    if (!options.manual && conversation.state !== "READY")
      throw new Error("CONVERSATION_NOT_READY");
    const sessionId = conversation.capture_session_id || conversation.id.replace(/^conv-/, "");
    const turns = this.db
      .query("SELECT * FROM turns WHERE session_id=? ORDER BY ordinal")
      .all(sessionId);
    const inputFingerprint = sha(
      turns.map((turn) => `${turn.id}:${turn.text_normalized}`).join("|")
    );
    const prior = this.db
      .query(
        "SELECT * FROM memory_extraction_runs WHERE conversation_id=? AND input_fingerprint=? AND prompt_version=?"
      )
    ...
    if (prior?.state === "COMPLETED" && !options.force) {
      return {
        runId: prior.id,
        cached: true,
        candidates: this.engine.listReviewInbox(workspaceId).memories,
      };
    }
    ...
    if (prior)
      this.db
        .query(
          "UPDATE memory_extraction_runs SET state='RUNNING',error_code=NULL,updated_at=? WHERE id=?"
        )
        .run(now, runId);
    else
      this.db
        .query(`INSERT INTO memory_extraction_runs
      (id,workspace_id,conversation_id,input_fingerprint,prompt_version,state,candidate_count,created_at,updated_at)
      VALUES (?,?,?,?,?,'RUNNING',0,?,?)`)
        .run(runId, workspaceId, conversationId, inputFingerprint, MEMORY_PROMPT_VERSION, now, now);
    try {
      const people = this.db
        .query("SELECT * FROM people WHERE workspace_id=? AND status!='ARCHIVED'")
        .all(workspaceId);
      const projects = this.db
        .query("SELECT * FROM projects WHERE workspace_id=? AND status!='ARCHIVED'")
        .all(workspaceId);
      const output = this.engine.candidateAdapter.extract({ turns, people, projects });
      const created = [];
      for (const candidate of output.candidates || []) {
        const validated = this._validateCandidate(
          workspaceId,
          conversationId,
          candidate,
          turns,
        ...
        const item = this._insertCandidate(
          workspaceId,
          conversationId,
          validated,
          inputFingerprint
      ...
      this.db
        .query(
          "UPDATE memory_extraction_runs SET state='COMPLETED',candidate_count=?,updated_at=? WHERE id=?"
        )
        .run(created.length, nowIso(), runId);
      this._audit({
        workspaceId,
        command: "EXTRACT_CANDIDATES",
        actor: "AI",
        after: { conversationId, created: created.map((x) => x.id) },
        idempotencyKey: `extract:${runId}`,
      });
      return { runId, cached: false, candidates: created };
    } catch (error) {
      this.db
        .query(
          "UPDATE memory_extraction_runs SET state='FAILED',error_code=?,updated_at=? WHERE id=?"
        )
        .run(String(error.message).slice(0, 120), nowIso(), runId);
      throw error;
    }
  }
  ...
  _validateCandidate(workspaceId, conversationId, candidate, turns, settings) {
    ...
    if (!SCOPE_TYPES.has(scopeType) || !MEMORY_TYPES.has(memoryType) || !content || !canonicalKey)
      return null;
    if (content.length > 400 || /[\r\n]{2,}/.test(content) || INJECTION_PATTERN.test(content))
      return null;
    this.engine._assertScope(workspaceId, scopeType, candidate.scopeRef);
    const sensitivity =
      String(candidate.sensitivity || "NORMAL").toUpperCase() === "SENSITIVE" ||
      SENSITIVE_PATTERN.test(content)
        ? "SENSITIVE"
        : "NORMAL";
    ...
    return {
      scopeType,
      scopeId: candidate.scopeRef,
      memoryType,
      canonicalKey,
      content,
      confidence: Math.max(0, Math.min(1, Number(candidate.confidence) || 0)),
      sensitivity,
      validFrom: candidate.validFromText || null,
      validUntil: candidate.validUntilText || null,
      evidence,
      conversationId,
    };
  }
  ...
  _insertCandidate(workspaceId, conversationId, candidate, inputFingerprint) {
    const fingerprint = sha(
      `${workspaceId}|${candidate.scopeType}|${candidate.scopeId}|${candidate.canonicalKey}|${normalizeContent(candidate.content)}`
    );
    const existing = this.db
      .query(`SELECT id FROM memory_items WHERE workspace_id=? AND scope_type=? AND scope_id=? AND fingerprint=?
      AND status IN ('CANDIDATE','CONFIRMED')`)
      .get(workspaceId, candidate.scopeType, candidate.scopeId, fingerprint);
    if (existing) return this.engine.getMemory(workspaceId, existing.id);
    const id = generateId("mem"),
      revisionId = generateId("mrev"),
      now = nowIso();
    this.db.transaction(() => {
      this.db
      ...
      this.db
        .query(`INSERT INTO memory_revisions
        ...
        VALUES (?,?,1,?,?,'EXTRACTED','AI','auralis-rule-extractor','deterministic-v1',?,?,?)`)
        .run(
          revisionId,
          id,
          candidate.content,
          JSON.stringify({ schemaVersion: 1, content: candidate.content }),
      ...
      for (const evidence of candidate.evidence) {
        this.db
          .query(`INSERT INTO memory_evidence
          ...
          .run(
            ...
            evidence.quote,
            now,
            sha(`${evidence.turnId}|${evidence.quote}`),
            now
          );
      }
  ...
  confirmMemory(workspaceId, id, options = {}) {
    ...
    this.db
      .query("UPDATE memory_items SET status='CONFIRMED',updated_at=? WHERE id=?")
      .run(nowIso(), id);
  ...
  rejectMemory(workspaceId, id, options = {}) {
    const current = this._get(workspaceId, id);
    this.db
      .query("UPDATE memory_items SET status='REJECTED',updated_at=? WHERE id=?")
      .run(nowIso(), id);
    this.engine.rebuildMemoryIndex(workspaceId);
    const updated = this._get(workspaceId, id);
    this._audit({
  ...
  editMemory(workspaceId, id, updates, options = {}) {
    ...
    if (
      ...
      Number(options.expectedRevision) !== Number(current.revision)
    ) {
      const error = new Error("CONCURRENCY_CONFLICT");
      error.status = 409;
      throw error;
    ...
    const revision = Number(current.revision) + 1,
      revisionId = generateId("mrev"),
      now = nowIso();
    const canonicalKey = updates.canonicalKey
      ? normalizeMemoryKey(updates.canonicalKey)
      : current.canonicalKey;
    const fingerprint = sha(
      `${workspaceId}|${current.scopeType}|${current.scopeId}|${canonicalKey}|${content}`
    );
    this.db.transaction(() => {
      this.db
        .query(`INSERT INTO memory_revisions
        ...
        .run(revisionId, id, revision, content, JSON.stringify({ schemaVersion: 1, content }), now);
      this.db
        .query(`INSERT INTO memory_evidence
        ...
        .run(
          ...
          content,
          now,
          sha(`USER_EDIT|${id}|${revision}|${content}`),
          now
        );
      this.db
        .query(
          `UPDATE memory_items SET canonical_key=?,current_revision_id=?,confidence=?,valid_from=?,valid_until=?,fingerprint=?,updated_at=? WHERE id=?`
        )
        .run(
          canonicalKey,
          revisionId,
          updates.confidence ?? current.confidence,
          updates.validFrom ?? current.validFrom,
          updates.validUntil ?? current.validUntil,
          fingerprint,
          now,
          id
        );
  ...
  archiveMemory(workspaceId, id, options = {}) {
    const current = this._get(workspaceId, id);
    this.db
      .query("UPDATE memory_items SET status='ARCHIVED',updated_at=? WHERE id=?")
      .run(nowIso(), id);
    this.engine.rebuildMemoryIndex(workspaceId);
    const updated = this._get(workspaceId, id);
    this._audit({
  ...
  deleteMemory(workspaceId, id, options = {}) {
    ...
    const now = nowIso();
    this.db
      .query("UPDATE memory_items SET status='DELETED',deleted_at=?,updated_at=? WHERE id=?")
      .run(now, now, id);
    ...
    if (!job) {
      const jobId = generateId("mpurge");
      this.db
        .query(`INSERT INTO memory_purge_jobs
      (id,workspace_id,memory_id,state,attempt,created_at,updated_at) VALUES (?,?,?,'QUEUED',0,?,?)`)
        .run(jobId, workspaceId, id, now, now);
      job = this.db.query("SELECT * FROM memory_purge_jobs WHERE id=?").get(jobId);
    }
    this._audit({
      workspaceId,
      memoryId: id,
      command: "DELETE",
      before: current,
      after: { status: "DELETED", purgeJobId: job.id },
      idempotencyKey: options.idempotencyKey,
    });
    if (options.purgeNow !== false) job = this._purge(job.id);
    return {
      memory: this.engine.getMemory(workspaceId, id, { includeDeleted: true }),
      purgeJob: job,
    };
  }
  ...
  _purge(jobId) {
    ...
    try {
      this.db
        .query(
          "UPDATE memory_purge_jobs SET state='RUNNING',attempt=attempt+1,updated_at=? WHERE id=?"
        )
        .run(now, jobId);
      const item = this.db
        .query("SELECT id FROM memory_items WHERE id=? AND status='DELETED'")
        .get(job.memory_id);
      ...
      if (item) {
        ...
        this.db
          .query(
            "UPDATE memory_revisions SET content_text='[purged]',content_json='{\"schemaVersion\":1,\"purged\":true}' WHERE memory_id=?"
          )
          .run(item.id);
      ...
      const exportsWithMemory = this.db
        .query(`
        ...
        try {
          ...
          if (payload.items && Array.isArray(payload.items)) {
            ...
            if (payload.items.length !== originalLength) {
              ...
              this.db
                .query(
                  "UPDATE memory_exports SET payload_json=?, item_count=?, updated_at=? WHERE id=?"
                )
      ...
      const remainingRevisions = this.db
        .query(`
      ...
      const remainingEvidence = this.db
        .query(`
      ...
      if (purgeFailures.length > 0) {
        ...
        this.db
          .query("UPDATE memory_purge_jobs SET state='FAILED',error_code=?,updated_at=? WHERE id=?")
      ...
      this.db
        .query(
          "UPDATE memory_purge_jobs SET state='COMPLETED',completed_at=?,updated_at=? WHERE id=?"
        )
  ...
  _detectContradictions(workspaceId, id) {
    const item = this.db
      .query(
        `SELECT m.*,r.content_text FROM memory_items m JOIN memory_revisions r ON r.id=m.current_revision_id WHERE m.id=?`
      )
      .get(id);
    if (!item || item.status !== "CONFIRMED") return [];
    const others = this.db
      .query(`SELECT m.id,r.content_text FROM memory_items m JOIN memory_revisions r ON r.id=m.current_revision_id
    ...
    for (const other of others) {
      if (normalizeContent(other.content_text) === normalizeContent(item.content_text)) continue;
      const [left, right] = [id, other.id].sort();
      const existing = this.db
        .query("SELECT id FROM memory_contradictions WHERE left_memory_id=? AND right_memory_id=?")
        .get(left, right);
      if (existing) continue;
      const cid = generateId("mcon"),
        now = nowIso();
      this.db
        .query(`INSERT INTO memory_contradictions
        (id,workspace_id,left_memory_id,right_memory_id,state,reason,confidence,created_at,updated_at)
        VALUES (?,?,?,?,'OPEN','same canonical key with different confirmed content',0.9,?,?)`)
        .run(cid, workspaceId, left, right, now, now);
      created.push(cid);
    }
    return created;
  }
  ...
  resolveContradiction(workspaceId, id, resolution, options = {}) {
    const row = this.db
      .query("SELECT * FROM memory_contradictions WHERE id=? AND workspace_id=?")
      .get(id, workspaceId);
    if (!row) throw new Error("CONTRADICTION_NOT_FOUND");
    const state = String(resolution || "").toUpperCase();
    if (!["RESOLVED_LEFT", "RESOLVED_RIGHT", "MERGED", "DISMISSED"].includes(state))
      throw new Error("CONTRADICTION_RESOLUTION_INVALID");
    const now = nowIso();
    this.db.transaction(() => {
      this.db
        .query(
          "UPDATE memory_contradictions SET state=?,resolved_by=?,resolved_at=?,updated_at=? WHERE id=?"
        )
        .run(state, "USER", now, now, id);
      if (state === "RESOLVED_LEFT")
        this.db
          .query("UPDATE memory_items SET status='SUPERSEDED',updated_at=? WHERE id=?")
          .run(now, row.right_memory_id);
      if (state === "RESOLVED_RIGHT")
        this.db
          .query("UPDATE memory_items SET status='SUPERSEDED',updated_at=? WHERE id=?")
          .run(now, row.left_memory_id);
      if (state === "MERGED")
        this.db
          .query("UPDATE memory_items SET status='SUPERSEDED',updated_at=? WHERE id=?")
          .run(now, row.right_memory_id);
  ...
  exportMemories(workspaceId, format = "BOTH") {
    const normalized = ["JSON", "MARKDOWN", "BOTH"].includes(String(format).toUpperCase())
      ? String(format).toUpperCase()
      : "BOTH";
    const id = generateId("mexp"),
      now = nowIso();
    this.db
      .query(`INSERT INTO memory_exports
      (id,workspace_id,state,format,item_count,created_at,updated_at) VALUES (?,?,'RUNNING',?,0,?,?)`)
      .run(id, workspaceId, normalized, now, now);
    try {
      const rows = this.db
        .query(`SELECT m.*,r.content_text,r.revision memory_revision FROM memory_items m
        JOIN memory_revisions r ON r.id=m.current_revision_id WHERE m.workspace_id=? AND m.status!='DELETED' ORDER BY m.updated_at DESC`)
        .all(workspaceId);
      ...
      this.db
        .query(
          `UPDATE memory_exports SET state='COMPLETED',payload_json=?,payload_markdown=?,item_count=?,completed_at=?,updated_at=? WHERE id=?`
        )
        .run(
          normalized === "MARKDOWN" ? null : JSON.stringify(payload),
          normalized === "JSON" ? null : markdown,
          items.length,
          nowIso(),
          nowIso(),
          id
      ...
      this._audit({
        workspaceId,
        command: "EXPORT",
        after: { jobId: id, itemCount: items.length },
        idempotencyKey: `export:${id}`,
      });
    } catch (error) {
      this.db
        .query("UPDATE memory_exports SET state='FAILED',error_code=?,updated_at=? WHERE id=?")
        .run(String(error.message).slice(0, 120), nowIso(), id);
    }
