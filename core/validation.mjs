...
export function assertSameWorkspace(db, workspaceId, table, idColumn, entityId) {
  ...
  if (!row) throw new ValidationError(`${table.toUpperCase()}_NOT_FOUND`, `${table} not found`, 404);
  if (row.workspace_id !== workspaceId) {
    throw new ValidationError('CROSS_WORKSPACE_REFERENCE', `${table} belongs to a different workspace`, 422);
  }
  return row;
}
...
export function assertValidEnum(value, allowedValues, fieldName) {
  if (!allowedValues.includes(value)) {
    throw new ValidationError('INVALID_ENUM', `Invalid ${fieldName}: ${value}. Allowed: ${allowedValues.join(', ')}`, 400);
  }
}

export function assertRevision(currentRevision, expectedRevision, entityName = 'Entity') {
  if (expectedRevision !== null && currentRevision !== expectedRevision) {
    const err = new ValidationError('CONCURRENCY_CONFLICT', `${entityName} was modified by another request`, 409);
    err.status = 409;
    throw err;
  }
}
...
export function assertDateTime(value, fieldName) {
  ...
  if (isNaN(date.getTime())) {
    throw new ValidationError('INVALID_DATETIME', `${fieldName} must be a valid ISO 8601 datetime`, 400);
  }
...
export function resolveSessionId(db, conversationId) {
  const conv = db.query('SELECT capture_session_id FROM conversations WHERE id = ?').get(conversationId);
...
export function sanitizeForAudit(obj) {
  ...
  const redactedKeys = ['content', 'content_text', 'content_json', 'exact_quote', 'quote', 'text_raw', 'text_normalized'];
