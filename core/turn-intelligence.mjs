...
const STOP_WORDS = new Set([
  'از','به','در','با','برای','که','این','آن','اون','یک','و','یا','را','رو','من','تو','ما','شما','او','هم','همین','همون',
  'است','هست','بود','شد','شود','شده','می','شود','کرد','کن','کنه','بده','بگو','لطفا','لطفاً','درباره','راجع','روی','بعد','قبل'
]);
...
function selectParent(previousTurns) {
  return [...previousTurns].reverse().find(turn =>
    turn && turn.id && ['question','request'].includes(String(turn.kind)) && String(turn.text_normalized || turn.text_raw || '').trim()
  ) || null;
}
...
export function analyzeTurn({ text, mode = 'study', sourceRole = 'manual', previousTurns = [] }) {
  ...
  const topicTerms = unique([
    ...quotedTerms,
    ...tokens.filter(token => token.length > 2 && !STOP_WORDS.has(token) && !/^\d+$/u.test(token))
  ]).slice(0, 10);
  ...
  const continuation = CONTINUATION_PATTERNS.some(pattern => pattern.test(normalized)) ||
    (tokens.length <= 5 && tokens.some(token => ['این','اون','همین','همون','بیشتر','ادامه'].includes(token)));
  ...
  const contextQuery = parent ? String(parent.text_normalized || parent.text_raw).trim().slice(0, 2_000) : '';
  ...
  const confidence = Math.max(0, Math.min(1,
    route.score + (topicTerms.length > 0 ? 0.08 : 0) + (parent ? 0.08 : 0) - (ambiguousContinuation ? 0.25 : 0)
  ));
