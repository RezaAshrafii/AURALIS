const SUPPORTED_SAMPLE_FORMATS = new Set(["pcm-u8", "pcm-i16", "pcm-i24", "pcm-i32", "float32"]);
...
export function inferSampleFormat(descriptor = {}) {
  const declared = String(descriptor.sampleFormat || descriptor.sample_format || "")
    .trim()
    .toLowerCase();
...
export function rawInterleavedToMonoPcm16(rawBytes, descriptor = {}) {
  ...
  for (let frame = 0; frame < frameCount; frame += 1) {
    ...
    for (let channel = 0; channel < format.channels; channel += 1) {
      mixed += decodeSample(
        raw,
        frameOffset + channel * format.bytesPerSample,
        format.sampleFormat
      );
    }
