import { generateId, nowIso, ALLOWED_COLOR_TOKENS } from './domain-models.mjs';
import { 
  ValidationError, 
...
export class WorkspaceService {
  ...
  // Workspaces
  listWorkspaces() {
    return this.db.query('SELECT * FROM workspaces ORDER BY created_at ASC').all().map(this._mapWorkspace);
  }
  ...
  createWorkspace(data) {
    ...
    const hasMemorySettings = this.db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='memory_settings'").get();
    if (hasMemorySettings) {
      this.db.query(`INSERT OR IGNORE INTO memory_settings
        ...
        VALUES (?,0,0,0,NULL,0,6,1800,1,?,?)`).run(id,now,now);
    }
  ...
  updateProject(id, data, expectedRevision = null) {
    ...
    const colorToken = data.colorToken !== undefined && ALLOWED_COLOR_TOKENS.includes(data.colorToken)
      ? data.colorToken
      : current.colorToken;
    const now = nowIso();
    const newRev = current.revision + 1;
  ...
  createPerson(workspaceId, data) {
    ...
    if (!displayName) throw new ValidationError('PERSON_NAME_REQUIRED', 'Person name is required', 400);
  ...
  unlinkPersonFromProject(projectId, personId) {
    this.db.query('DELETE FROM project_people WHERE project_id = ? AND person_id = ?').run(projectId, personId);
  }
