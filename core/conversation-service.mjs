...
export class ConversationService {
  ...
  listConversations(workspaceId, { projectId = null, limit = 50, offset = 0, status = null, includeArchived = false } = {}) {
  ...
  getConversation(id) {
    ...
    // Load participants
    conv.participants = this.db.query(`
  ...
  createConversation(workspaceId, data) {
    ...
    const kind = ['GENERAL', 'CALL', 'MEETING', 'INTERVIEW', 'NOTE'].includes(data.kind) ? data.kind : 'GENERAL';
    const state = ['DRAFT', 'STARTING', 'LIVE', 'PROCESSING', 'READY', 'FAILED', 'ARCHIVED'].includes(data.state)
      ? data.state
      : 'READY';
  ...
  deleteFinishedConversation(id, expectedRevision = null) {
    ...
    return this.updateConversation(id, { state: 'ARCHIVED', endedAt: current.endedAt || nowIso() }, current.revision);
  }

  addParticipant(conversationId, personId, participantRole = 'participant', speakerChannelId = null) {
    this.db.query(`
  ...
  removeParticipant(conversationId, personId) {
    this.db.query('DELETE FROM conversation_participants WHERE conversation_id = ? AND person_id = ?').run(conversationId, personId);
  }
  ...
  removeDocument(conversationId, documentId) {
    this.db.query('DELETE FROM conversation_documents WHERE conversation_id = ? AND document_id = ?').run(conversationId, documentId);
  }
  ...
  getAudioDetails(conversationId) {
    ...
    const channels = this.db.query('SELECT * FROM audio_channels WHERE session_id = ?').all(sessionId);
    const gaps = this.db.query('SELECT * FROM gaps WHERE session_id = ? ORDER BY created_at ASC').all(sessionId);
  ...
  getTranscriptTimeline(conversationId, { limit = 100, offset = 0, cursor = null } = {}) {
    ...
    const turns = this.db.query(sql).all(...params).map(r => ({
      id: r.id,
      ordinal: r.ordinal,
