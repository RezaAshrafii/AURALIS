import { normalizeText, escapeHtml } from './domain-models.mjs';
...
export class SearchService {
  ...
  rebuildIndex(workspaceId = null) {
    ...
    for (const per of people) {
      ...
      `).run(per.id, per.workspace_id, per.display_name, per.role_title || per.organization_name || '', `${per.email || ''} ${per.phone || ''} ${per.notes || ''}`);
    }
    ...
    for (const t of tasks) {
      this.db.query(`
        INSERT INTO search_projection (item_id, item_type, workspace_id, project_id, conversation_id, title, subtitle, body)
        VALUES (?, 'task', ?, ?, ?, ?, ?, ?)
      `).run(t.id, t.workspace_id, t.project_id || null, t.conversation_id || null, t.title, `${t.state} · ${t.priority}`, t.description || '');
    }

    // 5. Insights
    const insights = this.db.query('SELECT * FROM insight_items WHERE status != ?').all('DISMISSED');
    for (const ins of insights) {
      this.db.query(`
        INSERT INTO search_projection (item_id, item_type, workspace_id, project_id, conversation_id, title, subtitle, body)
        VALUES (?, 'insight', ?, NULL, ?, ?, ?, ?)
      `).run(ins.id, ins.workspace_id, ins.conversation_id, ins.title, `${ins.type} · ${ins.status}`, ins.body || '');
    }

    // 6. Documents
    const docs = this.db.query('SELECT * FROM source_documents').all();
    for (const d of docs) {
      const chunks = this.db.query('SELECT text_raw FROM source_chunks WHERE document_id = ?').all(d.id);
  ...
  search(workspaceId, query, { limit = 30, cursor = 0 } = {}) {
    ...
    const ftsQuery = q.split(/\s+/).map(term => `"${term.replace(/"/g, '""')}*"`).join(' AND ');
