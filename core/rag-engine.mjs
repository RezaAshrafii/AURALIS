...
const QUERY_STOP_WORDS = new Set([
  'از','به','در','با','برای','که','این','آن','اون','یک','و','یا','را','رو','من','تو','ما','شما','او','هم','است','هست','بود','شد','شود',
  'چی','چه','چرا','چطور','چگونه','کدام','میشه','می‌شود','لطفا','لطفاً','بگو','بده','کن'
]);
...
export function buildQueryPlan(query, { contextQuery = '' } = {}) {
  ...
  const primaryTerms = unique(tokenize(normalized).filter(term => term.length > 1 && !QUERY_STOP_WORDS.has(term))).slice(0, 18);
  const contextTerms = unique(tokenize(contextNormalized).filter(term => term.length > 2 && !QUERY_STOP_WORDS.has(term)))
...
export function chunkDocument(text, { targetChars = 1_100, overlapChars = 140 } = {}) {
  ...
  if (!Number.isInteger(targetChars) || targetChars < 300 || targetChars > 4_000) throw new TypeError('targetChars must be between 300 and 4000');
  if (!Number.isInteger(overlapChars) || overlapChars < 0 || overlapChars >= targetChars / 2) throw new TypeError('overlapChars must be non-negative and less than half targetChars');
  const chunks = [];
  let cursor = 0;
  while (cursor < source.length) {
    ...
    let end = targetEnd < source.length ? chooseBoundary(source, cursor, targetEnd, minimumEnd) : source.length;
...
function scoreCandidate(candidate, plan) {
  const text = normalizeFa(candidate.text_normalized || candidate.textNormalized || candidate.text_raw || candidate.textRaw || '');
  ...
  const lexicalCoverage = plan.primaryTerms.length ? primaryMatched.length / plan.primaryTerms.length : 0;
  const contextCoverage = plan.contextTerms.length ? matchedTerms.filter(term => plan.contextTerms.includes(term)).length / plan.contextTerms.length : 0;
  ...
  const ftsRank = Number.isFinite(Number(candidate.ftsRank)) ? Math.max(0, Number(candidate.ftsRank)) : 20;
  const ftsSignal = 1 / (1 + ftsRank);
  const score = lexicalCoverage * 0.56 + contextCoverage * 0.08 + Math.min(1, phraseHits) * 0.16 +
    Math.min(1, titleHits / Math.max(1, plan.primaryTerms.length)) * 0.08 + ftsSignal * 0.12;
  return {
    ...candidate,
    matchedTerms,
...
export function rankCandidates(candidates, plan, { limit = 8, maxPerDocument = 3 } = {}) {
  ...
  const sorted = [...deduped.values()].sort((a, b) =>
    b.retrievalScore - a.retrievalScore || Number(a.ftsRank || 0) - Number(b.ftsRank || 0) ||
