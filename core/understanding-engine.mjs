...
export class UnderstandingEngine {
  ...
  listInsights(conversationId, { type = null, status = null } = {}) {
    ...
    const items = this.db
      .query(sql)
      .all(...params)
      .map(this._mapInsight);
    for (const item of items) {
      item.evidence = this.db
        .query(`
  ...
  getInsight(id) {
    ...
    item.evidence = this.db
      .query("SELECT * FROM insight_evidence WHERE insight_id = ?")
      .all(id)
      .map((e) => ({
        id: e.id,
        insightId: e.insight_id,
  ...
  async runUnderstanding(conversationId, options = {}) {
    ...
    try {
      ...
      const extracted = await this._extractInsightsFromTurns(
        turns,
        conv.workspace_id,
        conversationId
      );
      ...
      const tx = this.db.transaction(() => {
        for (const item of extracted.items) {
          ...
          const fp = createHash("sha256")
            .update(`${conversationId}:${item.type}:${item.title}`)
            .digest("hex");
  ...
  async _extractInsightsFromTurns(turns, workspaceId, conversationId) {
    ...
    for (const turn of turns) {
      ...
      if (
        text.includes("تصمیم") ||
        text.includes("مقرر شد") ||
        text.includes("توافق شد") ||
        text.includes("تصویب")
      ) {
        items.push({
          type: "DECISION",
      ...
      if (
        text.includes("باید") ||
        text.includes("تسک") ||
        text.includes("پیگیری کن") ||
        text.includes("ارسال کن") ||
        text.includes("آماده کن") ||
        text.includes("بررسی شود")
      ) {
        let assigneeId = null;
      ...
      if (
        text.includes("قول می‌دهم") ||
        text.includes("من انجام می‌دهم") ||
        text.includes("متعهدم") ||
        text.includes("خودم تحویل می‌دهم")
      ) {
        items.push({
          type: "COMMITMENT",
          title: text.length > 50 ? text.slice(0, 47) + "..." : text,
