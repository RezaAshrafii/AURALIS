...
export class TranscriptRevisionAccumulator {
  ...
  get stableText() { return this.#stableText; }
  get finalText() { return this.#finalText; }
...
export function normalizeLoopbackBaseUrl(value) {
  ...
  let url;
  try { url = new URL(raw); } catch { throw new TypeError('local ASR URL is invalid'); }
...
export function extractWhisperCppText(payload) {
  ...
  if (typeof payload.transcription === 'string') return normalizeTranscriptText(payload.transcription);
  if (Array.isArray(payload.transcription)) {
    return normalizeTranscriptText(payload.transcription.map(item => item?.text || item?.content || '').join(' '));
  }
