...
const interrogatives = new Set([
  'چرا', 'چی', 'چه', 'کجا', 'کی', 'چند', 'چقدر', 'آیا', 'کدام', 'چطور', 'چگونه',
  'چجوری', 'چیه', 'چیست', 'کدوم', 'مگه', 'میشه', 'می‌شود', 'می‌شه', 'میتونی', 'می‌توانی'
]);

const requestPatterns = [
  'توضیح بده', 'دوباره بگو', 'شرح بده', 'تعریف کن', 'مقایسه کن', 'فرق', 'بگو',
  'بررسی کن', 'تحلیل کن', 'حل کن', 'مثال بزن', 'نشون بده', 'نشان بده', 'روشن کن'
];
...
export function routePersian(text, mode = 'study') {
  ...
  const shouldAnswer = Boolean(hasQuestionMark || hasInterrogative || hasRequest || hasQuestionForm);
  const kind = shouldAnswer ? (hasRequest && !hasQuestionMark ? 'request' : 'question') : 'statement';
  ...
  const featureCount = [hasQuestionMark, hasInterrogative, hasRequest, hasQuestionForm].filter(Boolean).length;
