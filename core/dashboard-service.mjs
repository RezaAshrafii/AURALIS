export class DashboardService {
  ...
  getDashboardMetrics(workspaceId) {
    ...
    const totalConvRow = this.db
      .query(
        "SELECT COUNT(*) as count FROM conversations WHERE workspace_id = ? AND state != 'ARCHIVED'"
      )
      .get(workspaceId);
    const totalConversations = totalConvRow ? totalConvRow.count : 0;

    // 2. Active projects
    const activeProjectsRow = this.db
      .query("SELECT COUNT(*) as count FROM projects WHERE workspace_id = ? AND status = 'ACTIVE'")
      .get(workspaceId);
    const activeProjects = activeProjectsRow ? activeProjectsRow.count : 0;

    // 3. People
    const peopleRow = this.db
      .query("SELECT COUNT(*) as count FROM people WHERE workspace_id = ? AND status = 'ACTIVE'")
      .get(workspaceId);
    const totalPeople = peopleRow ? peopleRow.count : 0;

    // 4. Tasks counts
    const openTasksRow = this.db
      .query(
        "SELECT COUNT(*) as count FROM tasks WHERE workspace_id = ? AND state IN ('TODO', 'IN_PROGRESS', 'SUGGESTED')"
      )
      .get(workspaceId);
    const openTasks = openTasksRow ? openTasksRow.count : 0;

    const completedTasksRow = this.db
      .query("SELECT COUNT(*) as count FROM tasks WHERE workspace_id = ? AND state = 'DONE'")
      .get(workspaceId);
    const completedTasks = completedTasksRow ? completedTasksRow.count : 0;

    const nowIso = new Date().toISOString();
    const overdueTasksRow = this.db
      .query(`
    ...
    const recentDecisionsCountRow = this.db
      .query(`
