...
export function runCitationBenchmark(cases, { minimumPrecision = 1, minimumQuoteCoverage = 1 } = {}) {
  ...
  const truePositive=results.reduce((sum,item)=>sum+item.actualChunkIds.filter(id=>item.expectedChunkIds.includes(id)).length,0);
  const falsePositive=results.reduce((sum,item)=>sum+item.actualChunkIds.filter(id=>!item.expectedChunkIds.includes(id)).length,0);
  const expectedValid=results.reduce((sum,item)=>sum+item.expectedChunkIds.length,0);
  const quotedValid=results.reduce((sum,item)=>sum+(item.expectedChunkIds.length?item.quoteCoverage*item.actualChunkIds.length:0),0);
  const precision = truePositive+falsePositive ? Number((truePositive/(truePositive+falsePositive)).toFixed(4)) : 1;
  ...
  return Object.freeze({
    schemaVersion:1,
    passed:passedCases === results.length && precision >= minimumPrecision && quoteCoverage >= minimumQuoteCoverage,
