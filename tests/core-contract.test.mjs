...
import { AnswerSchemaError, parseAnswerEnvelope } from "../core/answer-schema.mjs";
...
test("Persian router does not answer ordinary statements", () => {
  for (const s of [
    "امروز درباره واریانس صحبت کردیم",
    "کلاس ساعت ده شروع شد",
...
test("answer parser accepts direct JSON and filters citations", () => {
  const out = parseAnswerEnvelope(
    '{"answer":"پاسخ","sourceChunkIds":["a","x"],"grounding":"source"}',
    new Set(["a"])
  );
...
test("answer parser unwraps nested JSON answer envelope", () => {
  const nested = JSON.stringify({
    answer: JSON.stringify({ answer: "جواب نهایی", sourceChunkIds: ["c1"], grounding: "source" }),
    sourceChunkIds: [],
    grounding: "general",
  });
...
test("source grounding without an allowed citation is marked unverified", () => {
  const out = parseAnswerEnvelope(
    '{"answer":"پاسخ","sourceChunkIds":["fake"],"grounding":"source"}',
    new Set(["real"])
  );
...
test("runtime grounding envelope is accepted without source citations", () => {
  const out = parseAnswerEnvelope(
    JSON.stringify({
      answer: "این Turn از میکروفون رونویسی شده است.",
      sourceChunkIds: [],
      grounding: "runtime",
    }),
    new Set()
