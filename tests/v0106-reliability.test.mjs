...
import { isRuntimeCapabilityQuestion, shouldAutoAnswerTurn } from "../core/turn-policy.mjs";
...
test("meeting/oral ownership policy adapts safely when system audio is disabled", () => {
  assert.equal(
    shouldAutoAnswerTurn({ kind: "question", source_role: "system" }, "oral_copilot", {
      loopbackEnabled: true,
    }),
    true
  );
  assert.equal(
    shouldAutoAnswerTurn({ kind: "question", source_role: "user" }, "oral_copilot", {
      loopbackEnabled: true,
    }),
    false
  );
  ...
  assert.equal(
    shouldAutoAnswerTurn({ kind: "request", source_role: "manual" }, "oral_copilot", {
      loopbackEnabled: true,
    }),
    true
  );
  ...
  assert.equal(
    shouldAutoAnswerTurn({ kind: "statement", source_role: "system" }, "meeting"),
    false
  );
...
test("max-duration segmentation preserves one second overlap", {
