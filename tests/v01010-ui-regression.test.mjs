...
const react = await readFile(
  new URL("../apps/web/public/vendor/react.production.min.js", import.meta.url),
  "utf8"
);
const reactDom = await readFile(
  new URL("../apps/web/public/vendor/react-dom.production.min.js", import.meta.url),
  "utf8"
);
...
test("all product sections remain directly reachable", () => {
  for (const value of ["session", "sources", "settings", "system"])
    assert.ok(kit.includes(`value:'${value}'`), value);
  for (const label of ["جلسه", "منابع", "تنظیمات", "وضعیت سیستم"])
    assert.ok(kit.includes(label), label);
...
test("low-level telemetry stays out of the central conversation column", () => {
  ...
  for (const noisy of ["queueCapacity", "analysis:native.analysis", "component-grid"])
    assert.ok(!block.includes(noisy), noisy);
...
test("manual text turns join the same auto-answer policy when mode allows it", () => {
  assert.ok(
    server.includes(
      "shouldAutoAnswerTurn({kind:route.kind,source_role:'manual'}, session.mode, turnPolicyContext(sessionId))"
    )
  );
  assert.ok(server.includes("source:'manual'"));
});

test("focused workspace remains the active desktop layout without a visible technical rail", () => {
  assert.ok(css.includes(".focused-session"));
  assert.ok(css.includes(".focused-workspace"));
  assert.ok(app.includes("renderSession(){"));
  assert.ok(app.includes("className:'session-view focused-session'"));
});
