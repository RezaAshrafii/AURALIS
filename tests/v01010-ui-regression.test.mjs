...
test('release metadata identifies runtime auth stabilization while preserving focused workspace UI', () => {
  assert.ok(server.includes("const VERSION = '0.11.2'"));
  assert.ok(server.includes("RUNTIME_AUTH_STABILIZATION"));
  assert.ok(app.includes('focused-workspace'));
  assert.ok(html.includes('Auralis v0.11.2'));
});
