...
test("only packaged v0.14 bridge is promoted; v0.13 target artifacts remain explicit experimental inputs", () => {
  ...
  const fn = server.slice(
    server.indexOf("async function nativeExecutable()"),
    server.indexOf("async function startNativeCapture")
  );
  assert.ok(
    fn.indexOf("V014_NATIVE_CANDIDATES") < fn.indexOf("ENABLE_EXPERIMENTAL_V013_PRODUCT_CAPTURE")
  );
  assert.ok(
    fn.indexOf("ENABLE_EXPERIMENTAL_V013_PRODUCT_CAPTURE") < fn.indexOf("V013_NATIVE_CANDIDATES")
  );
