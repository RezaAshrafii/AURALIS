...
const hasProbeSource = [goUrl, vadUrl, audioFormatUrl].every((url) =>
...
test("raw capture path stays independent from VAD analysis", {
  skip: !hasProbeSource && "native-probe source is not shipped in the Portable package",
}, () => {
...
test("loopback silent advances are not mislabeled as loss", {
  skip: !hasProbeSource && "native-probe source is not shipped in the Portable package",
}, () => {
  assert.ok(go.includes("audio.silence_span"));
  assert.ok(
    go.includes("WASAPI loopback may not emit packets while the render endpoint is silent")
  );
});

test("derived VAD emits frozen segments and band-limited WAV", {
  skip: !hasProbeSource && "native-probe source is not shipped in the Portable package",
}, () => {
...
test("validation VAD exposes level telemetry", {
  skip: !hasProbeSource && "native-probe source is not shipped in the Portable package",
}, () => {
...
test("WAVEFORMATEXTENSIBLE parser uses byte offsets", {
  skip: !hasProbeSource && "native-probe source is not shipped in the Portable package",
}, () => {
