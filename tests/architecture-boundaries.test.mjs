...
import {
  ...
  readJsonBody,
  resolveStaticPath,
} from "../runtime/http-boundary.mjs";
...
test("runtime config is version-owned, loopback-only, and validates the port", async () => {
  ...
  const testConfig = await loadRuntimeConfig(root, {
    NODE_ENV: "test",
    AURALIS_TEST_PROVIDER_URL: "http://127.0.0.1:47999/v1/chat/completions",
  });
  ...
  await assert.rejects(
    () =>
      loadRuntimeConfig(root, {
        NODE_ENV: "test",
        AURALIS_TEST_PROVIDER_URL: "http://example.com/",
      }),
    /loopback/
  );
});
...
test("local request guard rejects cross-site bootstrap and uses authenticated state changes", () => {
  ...
  const local = new Request("http://127.0.0.1:47832/v1/bootstrap", {
    headers: { host: "127.0.0.1:47832" },
  });
...
test("task supervisor drains accepted background work and records failures", async () => {
  ...
  const supervisor = createTaskSupervisor({
    onError: (error, label) => errors.push([label, error.message]),
  });
  supervisor.run("failure", async () => {
