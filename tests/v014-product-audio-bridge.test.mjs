...
const runner = await readFile(
  new URL("../native/core/src/bin/auralis-audio-test.rs", import.meta.url),
  "utf8"
);
const persistence = await readFile(
  new URL("../native/core/src/audio/persistence.rs", import.meta.url),
  "utf8"
);
const app = await readFile(new URL("../apps/web/public/app-react.js", import.meta.url), "utf8");
const build = await readFile(
  new URL("../scripts/build-v014-windows-product-bridge.ps1", import.meta.url),
  "utf8"
);
const gate = await readFile(
  new URL("../scripts/run-v014-product-bridge-gate.ps1", import.meta.url),
  "utf8"
);
...
test("Rust stdout product protocol is strict session-bound JSONL", () => {
  ...
  assert.ok(
    runner.indexOf("journal.sync_data()") < runner.indexOf("let stdout = std::io::stdout();")
  );
});

test("server does not mark LIVE on process spawn and waits for every requested channel event", () => {
  const start = server.slice(
    server.indexOf("async function startNativeCapture"),
    server.indexOf("async function stopNativeCapture")
  );
  ...
  assert.ok(
    server.includes("expected.every(cid => nativeCapture.channels?.[cid]?.state === 'CAPTURING')")
  );
...
test("durable native chunks are integrity-checked, converted to WAV, and then frozen for ASR", () => {
  const materialize = server.slice(
    server.indexOf("async function materializeNativeChunkSegment"),
    server.indexOf("function ingestNativeEvent")
  );
  ...
  assert.ok(
    server.includes("eventJournals.push(join(sessionDir, runDir.name, 'product-events.jsonl'))")
  );
});
