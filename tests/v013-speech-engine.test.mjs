...
const migration = await readFile(
  new URL("../native/core/src/storage/migrations/0006_speech_engine.sql", import.meta.url),
  "utf8"
);
const repository = await readFile(
  new URL("../native/core/src/storage/repository.rs", import.meta.url),
  "utf8"
);
...
const runtimeStore = await readFile(
  new URL("../apps/web/src/runtime-store.ts", import.meta.url),
  "utf8"
);
...
test("stable transcript cannot regress and final transcript cannot be rewritten", () => {
  ...
  assert.equal(
    acc.accept({ state: "STABLE", text: "رگرسیون خطی", provider: "p", model: "m" }).accepted,
    true
  );
  assert.equal(
    acc.accept({ state: "STABLE", text: "متن دیگر", provider: "p", model: "m" }).reason,
    "STABLE_PREFIX_REGRESSION"
  );
  assert.equal(
    acc.accept({ state: "FINAL", text: "رگرسیون خطی ساده", provider: "p", model: "m" }).accepted,
    true
  );
  assert.equal(
    acc.accept({ state: "PARTIAL", text: "دوباره", provider: "p", model: "m" }).reason,
    "ALREADY_FINAL"
  );
});
...
test("local ASR endpoint is strictly loopback-only and strips URL decoration", () => {
  ...
  assert.equal(
    normalizeLoopbackBaseUrl("http://127.0.0.1:8080/inference?token=secret#x"),
    "http://127.0.0.1:8080"
  );
...
test("whisper.cpp response parser accepts common JSON response shapes", () => {
  ...
  assert.equal(
    extractWhisperCppText({ segments: [{ text: "سلام" }, { text: "دنیا" }] }),
    "سلام دنیا"
  );
...
test("neural VAD hysteresis suppresses short noise and closes after sustained silence", () => {
  const vad = new NeuralVadStateMachine({
    minSpeechMs: 60,
    minSilenceMs: 90,
    startThreshold: 0.6,
    endThreshold: 0.4,
...
test("shared contracts expose STABLE stream state without corrupting canonical final revision store", () => {
  assert.ok(
    contracts.includes("export type TranscriptStreamState = 'PARTIAL' | 'STABLE' | 'FINAL'")
  );
...
test("Rust core defines streaming transcript and neural VAD boundaries", () => {
  for (const needle of [
    ...
    assert.ok(rustAsr.includes(needle), needle);
  for (const needle of ["rest.contains('@')", "rsplit_once(':')", "host must be loopback"])
    assert.ok(rustAsr.includes(needle), needle);
  for (const needle of [
    "NeuralVadConfig",
    "VadProbabilityPort",
    "NeuralVadStateMachine",
    "SpeechStarted",
    "SpeechEnded",
  ])
    assert.ok(rustVad.includes(needle), needle);
});
