...
import { UnderstandingEngine, PROMPT_VERSION } from '../core/understanding-engine.mjs';
...
test('WorkspaceService handles CRUD, color tokens, and optimistic concurrency', () => {
  ...
  // 4. Create Project
  const project = service.createProject(ws.id, { name: 'سامانه تحلیل مکالمه', colorToken: 'purple' });
  assert.ok(project.id.startsWith('proj-'));
  assert.equal(project.colorToken, 'purple');

  // 5. Create Person
  const person = service.createPerson(ws.id, { displayName: 'رضا اشرفی', roleTitle: 'معمار سیستم', email: 'reza@example.com' });
...
test('SearchService indexes entities and performs full-text query', () => {
  ...
  wsService.createProject(ws.id, { name: 'پروژه یادگیری تقویتی', description: 'توسعه الگوریتم‌های PPO' });
  wsService.createPerson(ws.id, { displayName: 'دکتر حسینی', notes: 'متخصص یادگیری ماشین' });
  convService.createConversation(ws.id, { title: 'جلسه ارزیابی عملکرد مدل یادگیری', goal: 'بررسی دقت' });
  ...
  assert.ok(searchRes.results.length >= 3, 'Should find project, person, conversation, and task matching query');
...
test('Product router handles REST endpoints', async () => {
  ...
  const postWsReq = { method: 'POST', authenticated: true, json: async () => ({ name: 'فضای تستی جدید' }) };
  ...
  const postProjReq = { method: 'POST', authenticated: true, json: async () => ({ name: 'پروژه روتر', colorToken: 'emerald' }) };
  const postProjRes = await router(postProjReq, new URL(`http://localhost/v1/workspaces/${wsId}/projects`), jsonHelper);
  ...
  const dashRes = await router(dashReq, new URL(`http://localhost/v1/workspaces/${wsId}/dashboard`), jsonHelper);
...
test('Product UI keeps form submission and the first conversation turn reachable', async () => {
  ...
  assert.doesNotMatch(productApp, /disabled:!this\.state\.turns\.length,onClick:this\.openConversationHub/);
});
