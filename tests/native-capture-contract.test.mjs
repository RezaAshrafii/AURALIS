...
const sql = await readFile(
  new URL("../native/core/src/storage/migrations/0001_audio_ledger.sql", import.meta.url),
  "utf8"
);
const rustRepository = await readFile(
  new URL("../native/core/src/storage/repository.rs", import.meta.url),
  "utf8"
);

test("portable includes the compiled Windows capture probe", {
  skip: !hasProbeExe && "compiled probe is verified in Portable package",
}, async () => {
...
test("server exposes native capture start/stop/status and persistent chunk ledger", () => {
  for (const needle of [
    "/v1/native-capture/start",
    "/v1/native-capture/stop",
    "/v1/native-capture/status",
