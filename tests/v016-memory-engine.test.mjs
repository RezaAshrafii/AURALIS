...
import { MEMORY_PROMPT_VERSION, MemoryEngine } from "../core/memory-engine.mjs";
...
function fixture() {
  ...
  db.query(
    "INSERT INTO sessions(id,started_at,ended_at,mode,state,context_text) VALUES (?,?,?,?,?,?)"
  ).run("session-1", started, started, "study", "CLOSED", "");
  const conversation = conversations.createConversation(ws.id, {
    id: "conv-session-1",
...
function insertTurn(db, id, ordinal, text) {
  db.query(`INSERT INTO turns(id,session_id,ordinal,source_role,kind,text_raw,text_normalized,route_reason,route_score,state,created_at)
    VALUES (?,?,?,'manual','statement',?,?,'fixture',1,'COMMITTED','2026-08-23T10:00:00.000Z')`).run(
    id,
    "session-1",
    ordinal,
    text,
...
test("Consent is required and disabling memory stops extraction and retrieval immediately", () => {
  ...
  const enabled = memory.configureMemory(ws.id, {
    enabled: true,
    consent: true,
  ...
  assert.throws(
    () => memory.extractMemoryCandidates(ws.id, "conv-session-1", { manual: true }),
    /MEMORY_DISABLED/
  );
...
test("Candidate -> confirmed -> edited -> used -> deleted keeps provenance and audit", () => {
  ...
  assert.equal(
    db.query("SELECT prompt_version FROM memory_extraction_runs").get().prompt_version,
    MEMORY_PROMPT_VERSION
  );
  const candidate = extraction.candidates.find((item) => item.canonicalKey === "response.length");
  assert.equal(candidate.status, "CANDIDATE");
  assert.equal(
    candidate.revisions[0].evidence[0].exactQuote,
    "من جواب کوتاه و فارسی رسمی می‌خواهم."
  );

  const confirmed = memory.confirmMemory(ws.id, candidate.id);
  assert.equal(confirmed.status, "CONFIRMED");
  const context = memory.assembleMemoryContext(ws.id, "لطفاً پاسخ را کوتاه بده", {
  ...
  const edited = memory.editMemory(
    ws.id,
    candidate.id,
    { content: "پاسخ‌ها بسیار کوتاه و رسمی باشند." },
    { expectedRevision: 1 }
  );
  ...
  assert.equal(
    memory
      .queryRelevantMemories(ws.id, "پاسخ کوتاه")
      .memories.some((item) => item.id === candidate.id),
    false
  );
  const exported = memory.exportMemories(ws.id, "BOTH");
  assert.equal(
    exported.json.items.some((item) => item.id === candidate.id),
    false
  );
});
...
test("Contradictions remain independent until explicit resolution", () => {
  ...
  const second = result.candidates.find(
    (item) => item.scopeId === project.id && item.id !== first.id
  );
  memory.confirmMemory(ws.id, second.id);
  const contradiction = memory.listContradictions(ws.id, "OPEN")[0];
  assert.ok(contradiction);
  assert.equal(
    memory.queryRelevantMemories(ws.id, "وضعیت پروژه X", { scopeIds: [project.id] }).memories
      .length,
    0
  );
  memory.resolveContradiction(
    ws.id,
    contradiction.id,
    contradiction.leftMemoryId === first.id ? "RESOLVED_LEFT" : "RESOLVED_RIGHT"
  );
...
test("Workspace isolation and memory endpoint ownership block cross-workspace IDOR", async () => {
  ...
  const router = createMemoryRouter({
    memoryEngine: memory,
    readJsonBody: (req) => req.json(),
    requireState: (req) => req.authenticated === true,
  });
  const response = await router(
    { method: "GET", headers: new Headers({ "x-auralis-workspace-id": other.id }) },
    new URL("http://localhost/v1/memories/not-owned"),
...
test("Editable product entities use recoverable deletion and hide from active lists", () => {
  ...
  assert.equal(
    actions.updateTask(task.id, { title: "تسک ویرایش‌شده" }, task.revision).title,
    "تسک ویرایش‌شده"
  );
  assert.equal(actions.deleteTask(task.id).state, "CANCELLED");
  assert.equal(
    actions.listTasks(ws.id).some((item) => item.id === task.id),
    false
  );

  assert.equal(
    workspace.updateProject(project.id, { name: "پروژه ویرایش‌شده" }, project.revision).name,
    "پروژه ویرایش‌شده"
  );
  workspace.deleteProject(project.id);
  assert.equal(
    workspace.listProjects(ws.id).some((item) => item.id === project.id),
    false
  );

  assert.equal(
    workspace.updatePerson(person.id, { displayName: "محمد ویرایش‌شده" }, person.revision)
      .displayName,
    "محمد ویرایش‌شده"
  );
  workspace.deletePerson(person.id);
  assert.equal(
    workspace.listPeople(ws.id).some((item) => item.id === person.id),
    false
  );
  ...
  assert.equal(
    conversations.listConversations(ws.id).some((item) => item.id === finished.id),
    false
  );
  const live = conversations.createConversation(ws.id, { title: "مکالمه زنده", state: "LIVE" });
  assert.throws(
    () => conversations.deleteFinishedConversation(live.id),
    /CONVERSATION_NOT_FINISHED/
  );
});
...
test("Opt-in backfill is restart-safe, batched, progress-visible and pausable", () => {
  ...
  conversations.createConversation(ws.id, {
    id: "conv-session-2",
    captureSessionId: "session-2",
    title: "جلسه دوم",
    state: "READY",
    endedAt: "2026-08-23T11:10:00.000Z",
  });
  db.query(`INSERT INTO turns(id,session_id,ordinal,source_role,kind,text_raw,text_normalized,route_reason,route_score,state,created_at)
    VALUES (?,?,1,'manual','statement',?,?,'fixture',1,'COMMITTED','2026-08-23T11:00:00.000Z')`).run(
    "turn-backfill-2",
    "session-2",
    "من جواب کوتاه و فارسی رسمی می‌خواهم.",
    "من جواب کوتاه و فارسی رسمی می‌خواهم."
