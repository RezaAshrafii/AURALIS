...
test("Gemini authentication failures are actionable and do not masquerade as generic HTTP errors", () => {
  const out = classifyGeminiHttpError(
    401,
...
test("provider error sanitizer redacts API-key-looking material", () => {
  const out = classifyGeminiHttpError(
    400,
    JSON.stringify({ error: { message: "bad AIza123456789012345678901234567890" } }),
    null,
    "brain"
  );
...
test("runtime readiness requires enabled state and a configured credential", () => {
  assert.equal(
    runtimeCredentialReady({ enabled: true, hasCredential: true, lastState: "READY" }),
    true
  );
  assert.equal(
    runtimeCredentialReady({ enabled: true, hasCredential: false, lastState: "READY" }),
    false
  );
  assert.equal(
    runtimeCredentialReady({ enabled: true, hasCredential: true, lastState: "AUTH_REQUIRED" }),
    false
  );
});
